...
async function getAudioBuffer(arrayBuffer: ArrayBuffer, audioCtx: AudioContext, options = {}) {
  ...
  if (safari)
    return getAudioBuffer(arrayBuffer, audioCtx, { native: true })
  ...
  const chunk = { byteLength: 0, frames: [] }
  ...
  const length = audioBuffers.reduce((acc, current) => acc + current.length, 0)
